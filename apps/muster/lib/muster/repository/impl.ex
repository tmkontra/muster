defmodule Muster.Repository.Impl do
  ...
  def upload_layer_monolithic(upload_id, digest, blob, %__MODULE__{uploads: uploads, layers: layers} = state) do
  ...
  def upload_layer_chunk(upload_id, range_start, range_end, blob, %__MODULE__{uploads: uploads} = state) do
  ...
  def upload_manifest(reference, %{"layers" => manifest_layers} = manifest, manifest_digest, %__MODULE__{} = state) do
    case manifest_layers
      |> Enum.map(fn %{"digest" => digest} -> digest end)
      |> Enum.all?(&Map.has_key?(state.layers, &1))
      do
        true ->
