defmodule Muster.Repository.Server do
  ...
  @impl GenServer
  def handle_call({:upload_manifest, %ManifestUploadRequest{reference: reference, manifest: manifest, manifest_digest: manifest_digest}}, _from, state) do
  ...
  @impl GenServer
  def handle_call({:check_manifest, reference}, _from, %Repository.Impl{manifests: by_reference} = state) do
    exists? = Map.has_key?(by_reference, reference)
    {:reply, exists?, state}
  end

  @impl GenServer
  def handle_call({:get_manifest, reference}, _from, %Repository.Impl{tags: tags, manifests: by_reference = %{}} = state) do
